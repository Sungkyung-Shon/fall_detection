# app_run.py
# YOLOv8(상태 검출) → BoxMOT(추적) → (선택) ST-GCN(이벤트) → Late-Fusion → 알림/로그
...
def create_tracker(fps):
    ...
    try:
        return _Tracker(track_thresh=0.5, match_thresh=0.7, track_buffer=40, frame_rate=fps)
    except TypeError:
        try:
            return _Tracker(det_thresh=0.5, match_thresh=0.7, buffer_size=40, frame_rate=fps)
        except TypeError:
            return _Tracker()
...
def parse_args():
    ...
    # 입력/모델
    ap.add_argument("--model", default="/home/sung/runs/detect/train15/weights/best.pt")
    ap.add_argument("--source", default="0", help='0 또는 "video.mp4"')
    ap.add_argument("--imgsz", type=int, default=896)
    ap.add_argument("--conf", type=float, default=0.30)
    ap.add_argument("--fallen_name", default="fallen_person", help="모델 names에서 쓰러진 사람 라벨명")
    ...
    ap.add_argument("--stgcn_min_buf", type=int, default=12)  # 빠른 테스트용
...
def main():
    ...
    while True:
        ...
        # --- 3) 추적 ---
        tracks = tracker.update(dets_np, frame)  # [x1,y1,x2,y2,id,conf,cls,ind] (트래커 구현에 따라 다를 수 있음)
        ...
        for x1, y1, x2, y2, tid, conf, cls, _ in tracks:
            ...
            # p_state: 상태 신뢰도
            p_state = fallen_conf_from_dets(xyxy, dets_np, fallen_idx)
            ...
            if stgcn is not None:
                try:
                    ...
                    # (선택) 포즈 가시화
                    if args.show_pose and seq and len(seq) > 0:
                        kps = (seq[-1] * [W, H]).astype(int)  # (J,2)
                        for (kx, ky) in kps:
                            cv2.circle(frame, (int(kx), int(ky)), 2, (0, 255, 255), -1)
                except Exception:
                    p_event = 0.0
                    buf_len = 0
